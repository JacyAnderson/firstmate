...
| `fm-text-check.sh`       | List machine-writing tells in a diff's added comments and, with `--message-file` or a revision range, in the commit message; advisory unless `--strict` |
...
| `fm-guard.sh`            | Warn on primary-checkout tangles, pending queued wakes, stale watcher liveness, and daemonless away mode |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge and review-comment poll |
