...
On the markdown backend, retention (`done_keep`) moves answered rows out of the live backlog into the configured `[markdown] archive`, which `tasks-axi show` never reads.
Only when no live row carries an entry do `complete` and `verify` look for it in that archive, and they judge only the id's newest archived copy (the last one in the file): it must be closed and carry a recorded answer.
An older answered copy never satisfies a newer unanswered one, a newest archived copy without a recorded answer refuses by name, a live row (open, or closed without a recorded answer) is never satisfied from the archive, and a home whose `.tasks.toml` sets no archive key refuses exactly as it would without the lookup.
