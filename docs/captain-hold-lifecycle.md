...
When no live row carries an entry, or its live row is closed without a recorded answer, `complete` and `verify` look for it in that archive and accept only a closed archived row carrying a recorded answer, whichever of the id's archived copies holds it.
An archived row without a recorded answer refuses by name, an open live row is never satisfied from the archive, and a home whose `.tasks.toml` sets no archive key refuses exactly as it would without the lookup.
