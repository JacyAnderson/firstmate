...
FM_CHECK_INTERVAL=300   # seconds between slow checks (authenticated PR merge and review-comment polls, custom checks, or X-mode dispatch)
...
FM_SIGNAL_PROVENANCE_MAX_BYTES=131072   # size cap for the signal-provenance diary written by arm and away-mode daemon signal handlers
FM_SIGNAL_PROVENANCE_KEEP_LINES=500   # newest provenance lines kept when that diary is capped
