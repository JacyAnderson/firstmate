...
Keep the `[markdown] archive` key pinned: `bin/fm-captain-hold.sh` reads it to find answered captain calls that retention has rotated out of the live backlog, and without it the completion gate refuses its own answered calls ([captain-hold-lifecycle.md](captain-hold-lifecycle.md)).
...
FM_SPAWN_WORKTREE_TIMEOUT=60   # seconds a spawn waits for treehouse get to enter the task worktree; raise it when a project's first-ever worktree outlasts the default. The bound is a per-second poll count rather than strict wall clock: each poll costs one second of sleep plus the backend's own current-path probe, so on probe-based backends such as zellij and cmux the elapsed wait runs longer than the number. Unset or blank means 60; accepted values run from 2 to 86400, because the detection loop needs two polls to confirm the pane settled, and any other value refuses the spawn before a window is created. Only spawns that actually poll are validated: relaunches, --secondmate spawns, and the orca backend never run the poll and never consult the knob
...
FM_CHECK_INTERVAL=300   # seconds between slow checks (authenticated PR merge and review-comment polls, custom checks, or Relay dispatch)
