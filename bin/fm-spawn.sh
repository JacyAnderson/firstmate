...
# A relaunch reuses its recorded worktree, a secondmate owns its firstmate home
# directly, and the orca backend provisions its own worktree, so none of them
# calls `treehouse get` or polls for it. The predicate is computed once here and
# reused verbatim as that loop's own guard, so the wait budget's validation and
# the loop it guards cannot drift apart.
...
# Validated before any window, worktree, or task endpoint exists, so a bad value
# refuses with nothing to clean up rather than aborting mid-spawn or polling
# forever.
...
elif [ "$SPAWN_POLLS_WORKTREE" -eq 1 ]; then
