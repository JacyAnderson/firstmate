...
case "$MODE" in
  direct-PR)
    SETUP2=""
    RULE1='1. Never push to the default branch (push only your `'"$ID"'` branch). Never merge a PR.'
    DOD=$(cat <<EOF
# Definition of done
...
EOF
  ...
  local-only)
    SETUP2=""
    RULE1="1. Never push to any remote and never open a PR. Work only on your \`$ID\` branch; firstmate handles the merge into local \`main\`."
    DOD=$(cat <<EOF
# Definition of done
...
When it is implemented and committed, append \`done: ready in branch $ID\` to the status file and stop.
...
1. First action: create your branch: \`git checkout -b $ID\`$SETUP2
