...
# --- non-terminal stale, LIVE crew with a declared pause: absorbed, never an
#     ordinary stale wake --------------------------------------------------------
# Regression for the 2026-08-24 spurious wakes: healthy claude/tmux crews that
# appended `paused: <reason>` were stale-surfaced ~45-75s later on every pause,
# because pause_state_class required a confidently dead agent before honoring even
# an authoritative paused verdict from crew_absorb_class. A live idle agent is the
# EXPECTED shape of a deliberate external wait (the crew keeps its harness open
# while it waits), and the pause's own status append already surfaces once through
# the no-verb signal path, so the extra stale wake is pure duplication. The
# declared pause must be absorbed on the long cadence exactly as for a dead agent,
# and still re-surface once past FM_PAUSE_RESURFACE_SECS.
test_live_declared_pause_absorbed_then_resurfaced() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:fm-live-held"
  printf 'idle, waiting for the vendor rate-limit reset\n' > "$capture_file"
  printf 'window=%s\nkind=ship\nharness=claude\nbackend=tmux\n' "$window" > "$state/live-held.meta"
  statusf="$state/live-held.status"
  printf 'paused: waiting for the vendor rate-limit reset\n' > "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-live-held_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle, waiting for the vendor rate-limit reset")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"

  # Phase A: a fresh declared pause with a LIVE agent (pane_current_command is
  # the harness binary) must be absorbed: no wake, pause tracking recorded, no
  # wedge timer.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_FAKE_TMUX_CURRENT_COMMAND=claude \
    FM_FAKE_CREW_STATE='state: paused · source: status-log · waiting for the vendor rate-limit reset' \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_PAUSE_RESURFACE_SECS=999 FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher stale-surfaced a live crew's fresh declared pause (should absorb): $(cat "$out")"
  fi
  [ ! -s "$out" ] || fail "live declared pause printed a wake reason during absorb: $(cat "$out")"
  [ ! -s "$state/.wake-queue" ] || fail "live declared pause enqueued a wake during absorb"
  [ "$(cat "$state/.stale-$key" 2>/dev/null || true)" = "$pane_hash" ] || fail "stale suppressor not advanced on live paused absorb"
  [ -e "$state/.paused-$key" ] || fail "live declared pause did not record pause tracking"
  [ ! -e "$state/.stale-since-$key" ] || fail "live declared pause started the wedge timer"
  reap "$pid"

  # Phase B: the pause ages past the threshold; it re-surfaces once as a paused
  # recheck on the bounded cadence - never a wedge, never a bare stopped-crew wake.
  back=$(( $(date +%s) - 500 ))
  if [ "$(uname)" = Darwin ]; then touch -mt "$(date -r "$back" '+%Y%m%d%H%M.%S')" "$statusf"
  else touch -m -d "@$back" "$statusf"; fi
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-live-held_status"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 40 || fail "live declared pause did not re-surface past the threshold"
  grep -F "stale: $window" "$out" >/dev/null || fail "live pause re-surface did not print a stale recheck"
  grep -F "awaiting external" "$out" >/dev/null || fail "live pause re-surface was not the bounded paused recheck: $(cat "$out")"
  grep -F "possible wedge" "$out" >/dev/null && fail "live declared pause was mislabeled a possible wedge"
  [ -e "$state/.paused-resurfaced-$key" ] || fail "the paused re-surface throttle marker was not recorded"
  ...
  pass "a live crew's declared pause is absorbed like a dead one, then re-surfaced only on the bounded cadence"
}
...
# A still-live agent whose current state IS its declared pause is NOT a
# disconfirming case: it is the expected shape of a deliberate external wait, so
# it stays absorbed on the pause cadence (and discards any residual wedge timer)
# instead of surfacing an ordinary stale wake - see
# test_live_declared_pause_absorbed_then_resurfaced for the full live-crew
# regression.
test_exited_declared_pause_is_bounded_and_live_pause_stays_absorbed() {
  ...
  # First sight of a live crew whose current state IS its declared pause must be
  # absorbed onto the pause cadence, never surfaced as an ordinary stale wake.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live declared pause was stale-surfaced instead of absorbed: $(cat "$out")"
  fi
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live declared pause escalated on a residual wedge timer: $(cat "$out")"
  fi
  ...
  [ ! -e "$state/.wake-queue" ] \
    || wakes=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w { n++ } END { print n + 0 }' "$state/.wake-queue")
  [ "$wakes" -eq 0 ] || fail "live declared pause should stay absorbed, got $wakes stale wakes"
  pass "exited declared-pause and captain-held panes use bounded pause cadence and a live declared pause stays absorbed"
}
