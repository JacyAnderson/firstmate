...
Then append \`done [at=<epoch>]: PR {url} checks green\` and stop." \
...
test_repo_text_rule_is_read_from_owner_file() {
  ...
  out=$(FM_HOME="$home" FM_ROOT_OVERRIDE="$fake" "$fake/bin/fm-brief.sh" brief-norule-b firstmate --mode no-mistakes 2>&1); rc=$?
  ...
  for label in blank unterminated doubled; do
    ...
    out=$(FM_HOME="$home" FM_ROOT_OVERRIDE="$fake" "$fake/bin/fm-brief.sh" "brief-norule-$label" firstmate --mode no-mistakes 2>&1); rc=$?
...
EOF
  FM_HOME="$home" FM_ROOT_OVERRIDE="$fake" "$fake/bin/fm-brief.sh" brief-fixture-rule firstmate --mode no-mistakes >/dev/null 2>&1
...
test_ship_briefs_use_bare_slug_branch() {
  ...
  assert_grep "done [at=<epoch>]: ready in branch $id" "$brief" \
