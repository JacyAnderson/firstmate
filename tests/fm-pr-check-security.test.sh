...
# poll's comment path no-ops exactly as it does with no forge answer. They sit
# above the generic api cases, which would otherwise answer them.
case " $* " in
  *" api user "*)
...
test_comment_watch_watcher_and_rearm() {
  ...
  ack_watcher_cycle "$state" >/dev/null || fail "could not acknowledge the comment wake"
